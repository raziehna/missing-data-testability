...
# type_missing = args[[6]] # "SeqMNAR" vs "Permutation"
...
for (k in seq(K, 2)){
  ...
  # null 
  # ++++++++++++++++++
  model_null = glm(fmla_null[[k]], dat[idx, ], family = quasibinomial, weights=omega[idx]) 
  Wk_null = predict(model_null, dat, type = "response") # p(Rk = 1 | R_{\prec k}, XR_{\succ k}) 
  
  # ++++++++++++++++++
  # alternative 
  # ++++++++++++++++++
  model_alt = gam(fmla_alt[[k]], dat[idx, ], family = quasibinomial, weights = omega[idx])
  ...
  idx_alt = seq(1, n) # R_{\prec k} == 1
  for (j in seq(k-1, 1)){  
    idx_alt_t = which(R[,j] == 1)
    idx_alt = intersect(idx_alt, idx_alt_t)
  }
  
  Wk_null_natural[-idx_alt] = 1
  Wk_alt_natural[-idx_alt] = 1
  
  rho = -2*sum((1/omega[idx])*log(Wk_null_natural[idx]/Wk_alt_natural[idx]))
  pval = pchisq(rho, df = k-1, lower.tail=F)
  ...
  # update weights 
  # ++++++++++++++++++
  # W_model = glm(fmla_R[[k]], dat, family = binomial)  
  # W = predict(W_model, dat, type = "response")
  omega = omega*Wk_null
