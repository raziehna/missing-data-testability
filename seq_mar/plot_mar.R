...
process_df <- function(label, name="SeqMAR_continuous"){
  ...
  df_2 = read.csv(paste0(direct_2, "/", name, ".csv"), sep = ",")[-1, ]
  df_3 = read.csv(paste0(direct_3, "/", name, ".csv"), sep = ",")[-1, ]
...
plot_func <- function(plot_d, title){
  ...
  p + facet_grid(data_type ~ ., switch = "x") +
    ...
    scale_color_manual(labels = c("6%", "35%", "80%"),
                       values = c("aquamarine4", "coral3", "royalblue1")) 
  
}

plot_df_mar_cont = process_df(label="Sequential MAR does hold", name="SeqMAR_continuous")
plot_df_per_cont = process_df(label="Sequential MAR does not hold", name="Permutation_continuous")
plot_d_cont = rbind(plot_df_mar_cont, plot_df_per_cont)

plot_df_mar_bin = process_df(label="Sequential MAR does hold", name="SeqMAR_binary")
plot_df_per_bin = process_df(label="Sequential MAR does not hold", name="Permutation_binary")
